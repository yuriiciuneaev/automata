...
@section('content')
            ...
            @isset($services)
                ...
                <tbody id="myTbody">
                    @foreach ($services as $service)
                        <tr data-id="{{ $service->id }}" data-desc="{{ $service->description }}">
                            <td class="name" data-name="{{ $service->name }}">{{ $service->name }}</td>
                            <td class="filename"><a href="{{ url('/images/Services/' . $service->image) }}"
                                    target="_blank">{{ $service->image }}</a></td>
                            <td>
    ...
    <div id="addModal" class="modal bottom-sheet">
        <div class="modal-content">
            <h4>Add Product</h4>
            <div class="row">
                <form id="uploadForm" action="{{ route('services.post') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="row mb0">
    ...
    <div id="updateModal" class="modal bottom-sheet">
        <div class="modal-content">
            ...
            <div class="row">
                <form id="uploadForm" action="{{ route('services.put') }}" method="POST" enctype="multipart/form-data">
                    @method('PUT')
                    @csrf
                    <div class="row mb0">
...
@section('pagejs')
    <script src="{{ asset('js/pages/admin/services.js') }}"></script>
@stop
