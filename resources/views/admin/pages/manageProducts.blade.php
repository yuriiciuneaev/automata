...
@section('content')
            ...
            @isset($data)
                ...
                <tbody id="myTbody">
                    @foreach ($data as $product)
                        <tr data-id="{{ $product->id }}" data-desc="{{ $product->description }}">
                            <td class="name" data-name="{{ $product->name }}">{{ $product->name }}</td>
                            <td class="filename"><a href="{{ url('/images/Products/' . $product->image) }}"
                                    target="_blank">{{ $product->image }}</a></td>
                            <td>
    ...
    <div id="addModal" class="modal bottom-sheet">
        <div class="modal-content">
            <h4>Add Product</h4>
            <div class="row">
                <form id="uploadForm" action="{{ route('products.post') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="row mb0">
    ...
    <div id="updateModal" class="modal bottom-sheet">
        <div class="modal-content">
            ...
            <div class="row">
                <form id="uploadForm" action="{{ route('products.put') }}" method="POST" enctype="multipart/form-data">
                    @method('PUT')
                    @csrf
                    <div class="row mb0">
...
@section('pagejs')
    <script src="{{ asset('js/pages/admin/products.js') }}"></script>
@stop
